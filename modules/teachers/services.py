...
def count_employed_teachers(tenant_id: str) -> int:
    """Teachers whose employment is current — the same rule the list uses."""
    return (
...
def plan_teacher_headroom(tenant_id: str) -> Optional[int]:
    """Teacher seats left under the tenant's limit, or None when there is none.

    Counts employed teachers only, so a school is not held to its licence for
    teachers who have resigned, retired or been let go.
    """
    tenant = Tenant.query.get(tenant_id)
    if not tenant or tenant.max_employed_teachers is None:
        return None
    return tenant.max_employed_teachers - count_employed_teachers(tenant_id)
...
def _check_teacher_plan_limit(tenant_id: str) -> tuple:
    ...
    if headroom is not None and headroom <= 0:
        tenant = Tenant.query.get(tenant_id)
        return False, (
            f"Teacher limit reached for your plan (max {tenant.max_employed_teachers} "
            f"employed teachers). Contact support to upgrade."
        )
    return True, None
