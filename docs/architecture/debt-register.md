...
**Last updated:** 2026-09-14 — the leave module redesign
(`reviews/2026-09-14-leave-module-redesign.md`) made the principal's approval
stage reachable and rebuilt the mobile leave screens; 69 and 70 registered for
what it deliberately left. Earlier the same day, the notification pipeline
audit (`reviews/2026-09-14-notification-pipeline-audit.md`) fixed eight faults
across push, fan-out and the beat schedule; 65 and 66 registered for what it
deliberately left.

2026-08-09 — the Phase A stabilization audit
...
|---|------|---------------|------|
| 69 | **`/api/holidays` REST reads are not narrowed to the caller, while the calendar's own reads are.** ADR-025 made every academic-calendar read answer from a `CalendarAudience`, so a closure addressed to staff (`holidays.applies_to = 'staff'`) is correctly absent from a student's calendar, their day feed and their working-day count. The REST list the same rows are also served from — `GET /api/holidays/`, `/upcoming`, `/recurring` (`modules/academics/calendar/holiday_routes.py`) — applies no such filter, so the same student reading the same closure through that endpoint sees it. The Expo client reaches it on two screens (`AdminAttendanceScreen`, `MarkAttendanceScreen`) and in `teacher-leaves/utils/workingDays.ts`. The exposure is mild — a school closure is not another student's data — but the two transports now disagree about the same question, which is exactly what the v2 rule against two transports per operation exists to prevent. | attendance and leave working-day math read this list, and narrowing it by `applies_to` would change what counts as a working day for attendance — a behavioural change to a shipped module, made from inside a calendar change, with no test asserting the current arithmetic | decide what a partial-audience closure means for attendance first (is a staff-only closure a working day for the register?), then either narrow the REST reads to match the calendar or retire them in favour of the GraphQL holiday fields, which the Expo client can now reach — it gained a `gql()` helper in this change |
| 68 | **One markdown-stripping rule now lives in three languages across three repositories, and only two of them can be tested.** A notification body is a plain-text surface, so `server/modules/announcements/markdown_text.py` flattens it before storage; rows written before that still carry raw markdown, so `admin-web/src/lib/notificationBody.ts` and `client/modules/notifications/formatNotificationBody.ts` each strip it again at render time. The three carry the same list of markup forms — nested emphasis, bullets, ordered lists, headings, blockquotes, links, images, strikethrough, inline and fenced code, thematic breaks — and nothing checks that they agree. Worse, only two are covered: the server has `test_markdown_text.py` and admin-web has `notificationBody.test.ts`, while **the Expo client has no test harness at all** — no runner, no config, no test file anywhere in the repository — so the copy that recipients actually read is the one nothing asserts against. Found during the 2026-09-14 notification fixes; verified by compiling the module with `tsc` and exercising it under `node` by hand, which is not a regression test. | the three surfaces are in three repositories with no shared package between them, and adding a test runner to the Expo app was a larger change than the bugfix that needed it | the client half closes when Expo gets a test runner (jest-expo is the conventional choice and `client/` has none); the duplication closes properly only if the stripper becomes something shared — either a published package or, more cheaply, a single fixture file of input/expected pairs committed once and read by all three suites, so a form added to one is a failing test in the others rather than a silent divergence |
| 67 | **Building a Flask app repoints every Celery task at it, through a module global.** `create_app()` (`app.py:561`) calls `init_celery(app)`, which assigns the module-global `_celery` in `celery_app.py`; its `ContextTask.__call__` runs every task body inside `with app.app_context()`. So constructing a second app — which is the obvious thing to do in a test that wants to inspect configuration — silently rebinds task execution to that app's engine registry. In tests that means the real database instead of the fixture's transactional connection, and the failure surfaces in whatever runs *next*: found 2026-09-14 when `test_beat_schedule.py` called `create_app()` and the notification tests that sort after it alphabetically began reporting `no_user` and recipients stuck at `pending`. Reproducible only under `-p no:randomly`; invisible when the affected files run alone. Worked around by having the test read `get_celery()` rather than build an app, which leaves the global itself as sharp as it was. | the app factory needs to hand Celery a configured app, and a module global was the shortest path from `create_app` to `celery -A celery_worker:celery`, which imports the module rather than receiving anything | make the binding explicit rather than ambient — either have `celery_worker.py` own the single `init_celery` call and let `create_app` stop mutating the global, or key the app off the task at call time so a second app cannot capture tasks it did not create. Until then a conftest guard that fails any test constructing a second app would catch the next instance at its own name instead of a stranger's |
| 66 | **Sending a notification requires the finance manage permission.** `PERM_MANAGE = "finance.manage"` gates both `POST /api/notifications/send` and `/send-bulk` (`modules/notifications/routes.py`). Nothing about a general notification is financial, and the RBAC catalogue already carries `announcement.create` — "Create announcements as an admin" — which is what this actually is. So the head who may address the whole school cannot, unless somebody also gave them fee rights; and anybody with fee rights can address the whole school. | the send endpoints predate the announcements permission set, and `finance.manage` was the nearest thing that existed when they were written | decide whether these endpoints survive at all now that announcements own bulk messaging — if they do, move them to `announcement.create`, and check which live roles hold `finance.manage` but not `announcement.create` before flipping it |
| 65 | **A transient push failure is lost, and the recipient row says it was sent.** Two halves of one gap, found in the 2026-09-14 notification audit. (a) `deliver_to_tokens` retries a failed token exactly once, immediately — no backoff, microseconds later — so a network blip against FCM or Expo fails both attempts; it then returns counts normally rather than raising, so `send_push_task`'s own `retry_backoff=True` never engages and the push is simply gone. (b) `PushStrategy.send` returns `True` for "enqueued", which is the honest answer for an async channel but means `process_notification_chunk`'s `ok = any(results.values())` marks the recipient `sent` whenever `PUSH` is in the channel list, whatever happened downstream. Together: a push can vanish and the row will claim delivery. | the inner retry was written before `send_push_task` had Celery-level backoff, and the two now overlap without composing; the strategy's return value genuinely cannot know a delivery outcome from inside an enqueue | give delivery a real outcome to record — either raise from `deliver_to_tokens` on an all-transient failure so Celery's backoff owns the retry (needs per-token idempotency first, or successful tokens get a duplicate on retry), or have `send_push_task` write the outcome back and stop treating an enqueue as evidence. Decide alongside whether `notification_recipients.status` is meant to track in-app state or delivery across all channels — today it tries to be both |
| 64 | **`plans` and `tenants.plan_id` are now unreferenced and should be dropped.** Migration 043 moved pricing and feature flags onto the tenant and kept `plans` only until nothing read it. The last readers were the student and teacher creation limits; migration 137 moved those onto the tenant too (`max_active_students`, `max_employed_teachers`, backfilled from each tenant's plan) and the panel edits them on the pricing card. The table, the FK, the `Plan` model and the `Tenant.plan` relationship remain in the tree with no caller. | dropping a table in the same change that moved its last readers would have made the backfill and the drop one irreversible step; the model and relationship stay so the backfill can be re-run against a restored database | a migration that drops `fk_tenants_plan_id`, `tenants.plan_id` and `plans`, plus deleting `Plan` and `Tenant.plan` from `core/models.py`, once 137 has run in production |
| 63 | **Biometric unlock is a device-local convenience with no school policy over it.** The Expo app gates re-entry into an existing session behind Face ID / fingerprint (`client/modules/auth/biometrics/`). It is not an authentication method: the server never learns biometry happened, no strategy is registered, and `tenant_auth_policy_rules` is untouched. So a school that wants to forbid it — or to require it — has no way to say so, and no way to see who has turned it on. | modelling a device-local convenience as a login method would put a fifth row in the strategy registry for something the server cannot verify, which is worse than not having the control; the `surface` column on `tenant_auth_policy_rules` is ready if a customer asks | when a school actually asks: either a scalar on `tenant_auth_policies` (simplest, and honest about it not being a method), or revisit as part of `passkey`, which *is* a method and would carry the policy naturally |
| 62 | **Nothing enforces that a commit dominates every early return after a flush-only service call, and a second, older convention says the opposite.** Phase 9's service layer (`modules/integrations/`, `modules/auth/otp.py`, `modules/auth/policy.py`) flushes and leaves the commit to the route, on the reasoning that a route may need to compose several service calls into one transaction. That convention is nowhere written down, and it coexists, undocumented, with an older one where a model owns its own persistence — `User.save()`, `Role.save()`, `TenantIntegration`'s siblings across `modules/attendance/models.py`, `modules/rbac/models.py`, `modules/classes/models.py`, `modules/teachers/models.py` and more all call `db.session.commit()` on themselves. A reader moving between the two has no way to tell, from the call site alone, whether the object in front of them just persisted or is waiting on someone else to. Phase 9 found this the hard way, twice, with nothing but a real walkthrough — no test caught either: `POST /api/auth/otp/request` created a challenge, sent the real code, and returned success while never committing (`challenges` count stayed at zero against the running stack — mobile OTP could not succeed for anyone, failing as a generic wrong-code 401); and `_login_through_pipeline` committed on the success path only, so `auth_events` rows for a wrong password, a lockout, a policy denial or an unresolved tenant were silently rolled back — the exact audit trail `.claude/rules/security-guardrails.md` requires for authorization failures. **A third instance, found 2026-09-08 and worse than either:** `create_session` committed the `Session` through `Session.save()` — the older self-committing convention — and then minted the refresh token with `add` + `flush` under the newer one, with `_finalize_login` committing nothing after. The verifier row was discarded at teardown, so **every session on every surface died with its first access token**, fifteen minutes in, and the fix was one commit. This is the single clearest case for closing this debt: the two conventions did not merely coexist confusingly, they met inside one function and the boundary between them was where the data was lost. Caught by reading the database (`refresh_tokens` held 0 rows against 22 live sessions), not by 3,300 passing tests — which passed because they assert against the same session the request used, where a flushed row is visible. Closed for this call site by `tests/auth/test_refresh_token_survives_the_request.py`, which reads across `db.session.remove()`; the convention itself is still unwritten. | Phase 9 needed a route to commit several service calls as one transaction, which requires services that flush and leave committing to the caller — the right shape for that need, adopted with nothing to stop an early return in the caller from skipping the commit it owns, and nothing flagging that an older, fully self-committing convention already existed alongside it | pick one convention or document where each applies (e.g. "a service that composes with others in one transaction flushes; a service that is always its own transaction commits"), then add a lint or a test — shaped like a walk over routes that call a flush-only service checking for a commit on every return path — that would have caught D1 and D2 before a human had to find them by using the app |
| 61 | **No messaging capability bills on a fresh deployment — not just `whatsapp`.** Verified independently: `ProviderService(` and `TenantService(` are constructed nowhere in `modules/`, `migrations/` or `scripts/` outside `modules/billing/models.py`'s class definitions and `modules/billing/services.py`'s admin API — no migration, no seeder, no onboarding path creates a catalog row or a subscription for `sms`, and the only place an `sms` `ProviderService` exists anywhere in this repository is a pytest fixture. The line in `integrations.md` this debt should have matched all along is its own: "Phase 2 shipped the billing catalog empty ... nothing seeds it." `usage_recorder.record_provider_usage` calls `record_usage(service_key=capability)` (`modules/billing/usage.py`), which resolves the school's `TenantService` for that key and **raises `UnknownService`** when none matches. `record_provider_usage` catches exactly that exception, logs it at `ERROR`, and returns `None` rather than propagating — deliberately: a failure to record must not turn an already-sent message into a reported failure, which would invite a retry and a second charge. That is the right call for *send* correctness, and its cost is that an unbilled message is silent by design — nothing pages on the ERROR line, and nothing distinguishes "this school genuinely was not sold the service" from "nobody has populated the catalog yet." Net effect on a fresh deployment: a school can be switched onto `mobile_otp`, send OTPs — or WhatsApp messages — for an entire term, and be invoiced for none of them, with the only trace an ERROR log line nobody is watching. WhatsApp is not a special case of this; it is the same gap on the newer capability, surfaced because Task 11 went looking at exactly the moment WhatsApp shipped with no catalog entry either. | the billing catalog was built as an operator-populated registry (Phase 2), correctly — it is commercial data, not something safe to seed with a guessed price — but nothing since has made populating it, for either capability, part of any deployment or onboarding checklist, so it silently stayed empty in every environment that never had an operator do it by hand | for a given school and capability, both `POST /platform/service-catalog/services` (`upsert_provider_service`) — add the capability as a service some provider sells, once per provider, not per school — and `POST /platform/tenants/<id>/services` (`configure_tenant_service`) — subscribe that school to it — must be done before any usage records. Neither exists today for `sms` or `whatsapp`. Nothing currently enforces this: an operator can enable an integration and a school can send real, billable-looking messages with no catalog entry behind either, and nothing warns them at enable time, at send time, or anywhere an operator would see it before the bill comes up short |
| 60 | **A model that derives a CHECK constraint from a live constant will diverge from the database the moment the constant changes, with nothing to catch it.** `modules/integrations/models.py` builds `ck_tenant_integrations_capability` dynamically from `CAPABILITIES`. Migration 129 froze the same constraint as a literal at authoring time (`capability IN ('sms')`) — that is correct; a migration is supposed to record one moment, and 129 recorded its moment right. The divergence came from the model's side: when Task 2 added `whatsapp` to `CAPABILITIES`, the model's constraint widened in Python with no matching migration, and the database kept enforcing only `sms` — silently, because nothing read the deployed constraint back to check it still agreed with the model. Closed for this column by migration 135 (widens the frozen constraint to `capability IN ('sms', 'whatsapp')`) and by `tests/test_integration_capability_constraint.py`, which reads the constraint's actual definition off `pg_constraint` rather than off the model — asserting against the model would only prove the model agrees with itself, which is the blind spot that let this happen in the first place. The same *style* — a migration building a CHECK with an f-string over a module-level tuple, frozen correctly the same way 129 was — also appears in `080_staff_relationship.py`, `128_what_a_school_pays_for_and_what_it_costs_us.py` (twice), `130_a_code_that_lives_for_five_minutes.py`, and 129's own sibling `ck_tenant_integrations_status`; none of these is itself a latent bug, but each has a model that could do to it what `TenantIntegration`'s did. | a model was written to track a live constant so its own `CHECK` could never look stale in code, and nothing then confirmed the frozen migration still matched it once the constant moved | give every status/enum column whose model builds its `CHECK` dynamically a test shaped like `test_integration_capability_constraint.py`'s — read the constraint back off `pg_constraint` and compare it to the live constant. Rewriting the other migrations' f-strings as plain literals is a readability nit, not a fix, and can be done opportunistically |
| 56 | **The platform dashboard's monthly revenue is an annual figure divided by twelve.** `monthly_run_rate` (`modules/billing/calculation.py`) is what `get_dashboard_stats` has always done, now named instead of written inline. There is no monthly billing behind it: `BILLING_CYCLES` has one member, no proration exists, and a school that joined in November is counted as though it had paid all year. Phase 2 kept the number exactly as it was rather than moving a figure somebody may be reading. | consolidating the arithmetic was in scope; changing what the dashboard reports was not | decide what "monthly revenue" should mean — recognised revenue, run rate, or nothing — and either model it or relabel the tile |
| 57 | **There is no correlation id, so a provider call cannot be traced back to the request that caused it.** `.claude/rules/api-conventions.md` specifies `X-Request-Id` on every response and `meta.requestId` in every payload; nothing generates one, `X-Request-Id` is in neither `CORS_ALLOW_HEADERS` nor `CORS_EXPOSE_HEADERS`, and there is no logging configuration at all — no `dictConfig`, no formatter, no structured output. Phase 3 mints a per-call `operation_id` (`modules/integrations/operations.py`) so an integration call and its usage record can be tied together, which is the smallest thing that made the phase debuggable. | implementing the documented convention is an observability project, and the integration layer needed one identifier, not a logging stack | generate a request id in `app.py` alongside the existing `Cache-Control`/`X-Feature-Stamp` after-request hooks, expose it, put it in `meta`, and let `operation_id` carry it instead of minting its own |
| 58 | **A household cannot share a sign-in number, and the schema and its own comments disagree about whether it should.** `uq_account_identifiers_live` is UNIQUE on `(tenant_id, identifier_type, identifier_value_normalized)` for live rows, so two accounts at one school cannot both hold `+919876543210` — while the comment three lines above it says "a household phone number will legitimately appear in two", and the strategy registry says the same. Real migrated data makes this concrete: the scale fixture models one household in five sharing a placeholder number, and a father and a mother are two `Person` rows carrying one number by design. Phase 4 kept the constraint, refused the collision loudly (`MobileAlreadyInUse`) and made resolution handle the plural case anyway — it returns every match and signs in nobody when there is more than one — so relaxing the index later cannot silently become an arbitrary choice. | whether a shared number should resolve to several accounts is a product decision, and taking it inside an implementation phase would have constrained parent accounts before they are designed | decide with parent accounts: either one account per number (and `family_access_mode` says whose), or relax the index and add an account-selection step after the code is verified — the caller has proved possession of the phone by then, so listing the accounts on it is defensible |
| 59 | **The server still renews a session from a header no client sends.** `core/authentication.py` accepts `X-Refresh-Token` on any request and renews in passing, announcing the result in `X-New-Access-Token` / `X-New-Refresh-Token`. Phase 8 moved every client onto `POST /api/auth/refresh`, because a rotating refresh token cannot ride along on twenty parallel requests without looking like theft. The old path is kept so a build already on a phone keeps working. | the Expo repository does not deploy on a `main` push unless the commit says `[store-release]` or `[ota]`, so an old build can outlive the server change | once the mobile release carrying Phase 8 is the minimum supported build, delete the header branch and the two response headers |
| 48 | **No AssessmentScheme, and the reuse question is unanswered.** Theory 80 + Practical 20 is two `exam_papers` (ADR-016), which is what a school sits — but a school repeating that shape across Unit Test 1, Half Yearly and Final re-enters it each time. A scheme would be a *template that stamps papers out*, addable without touching `exam_papers`. | building it now meant guessing reuse rules before a school expressed one | decide when a school asks; the model does not block it |
| 52 | **Weighted aggregation is unbuilt, and `paper.weight` blocks calculation until it is.** EX-03A refuses any result whose papers carry a non-null weight (`WEIGHTED_CALCULATION_UNSUPPORTED`, ADR-020 §C). `weight` is the one paper column `add_papers` accepts with **no validation at all** — unlike `max_marks` and `pass_marks` it gets no numeric, range or finite check, and `numeric` will store `NaN` in it. So a school that has set weights cannot compute results, deliberately: ignoring the column would hand them an unweighted number they did not ask for, and consuming it would run arithmetic on a value nothing vetted. | the mathematics were never decided and the column was never validated | decide what a weight means (share of a subject total? of the examination?), whether weights may be partial across papers, and whether they must sum to 100 — then validate the column in `_add_papers_impl` in the same slice that consumes it |
| 53 | **Grading bands have no integrity constraints, and their bounds are coarser than the percentages matched against them.** Only `min_value <= max_value` is enforced. *(EX-03B, 2026-08-16: found a second, sharper half — `grading_bands.min_value`/`max_value` are `Numeric(6,2)` while `exam_results.percentage` is `Numeric(6,3)`. A school writing the common "up to 59.999" upper bound gets **60.00** stored, which silently overlaps the band starting at 60. The overlap is unauthored and invisible: it resolves by lowest `sequence` like any other, so a student on the boundary quietly takes the higher band.)* Gaps, overlaps and duplicate boundaries are all storable, and a shipped test fixture already contains a gap (`A1 91-100, A2 81-90.99, F 0-32.99` leaves 33-80.99 uncovered). EX-03A behaves defined-ly against both — a gap yields no grade plus a snapshot warning, an overlap resolves by lowest `sequence` — but nothing stops a school configuring either, and no screen tells them they have. | bands are configuration and the catalogue shipped before anything read it | validate coverage when grading configuration gets a surface: warn on gaps, refuse overlaps, or both. Do not repair existing rows silently |
| 54 | **ADR-019 is cited but was never written.** `modules/examinations/models.py:162` justifies `grading_schemes` being tenant-wide with "(ADR-019)", and no such file exists. The decision itself is real and was taken — the discovery audit's D3, "grading scope: (a) tenant-wide in MVP, structured for (b) per programme/board" — it simply never became a document, so the code cites a rule a reader cannot check. | the decision was recorded in an audit table rather than an ADR | write ADR-019 from audit §20 D3 when grading scope is next touched, or correct the citation to point at the audit |
| 51 | **How many students were never marked is unrecoverable once a paper locks.** A locked paper's cohort is its own `ExamMark` rows (EX-02A.1), which is what makes a historical register survive promotion, section transfer and withdrawal — but it means a paper closed with 33 of 35 marked reports 33, and `marking_progress` returns `outstanding = None` rather than a zero that would read as "everybody was marked". The two missing students are genuinely not stored anywhere: enrollment has moved on, and the alternative was to invent them from today's class. | the honest options were a wrong number or a snapshot column, and a migration was out of scope for the slice | if a school needs the count, snapshot the cohort size (or the cohort) on `exam_papers` at lock time — decide with EX-02B, when the correction workflow shows whether the missing rows matter |
| 49 | **Report card vs marksheet is undecided.** `exam_results` is one examination's outcome for one student. An annual report card aggregating unit tests + half yearly + final is a different artefact that would *read* results, not replace them. Nothing prevents it and nothing implements it. | a product decision, not an architectural one | answer before the first result document is designed |
...
| 46 | **Three GraphQL queries no client selects.** Of 40 root queries, `me`, `student` and `recurringHolidays` appear in no client document — checked by parsing every backtick GraphQL document in `admin-web/src`, `panel`, `client/modules` and `client/app` (107 documents) and reading their root selections, not by grepping the field name, which matches the word anywhere. Each is unreached for its own reason and none is simply deletable: **`me`** returns the v2 person-centric identity but not the permissions, tenant and setup flags `AuthProvider` reads, so `/api/auth/profile` cannot move until it carries them; **`student`** returns the 13 list-shaped fields of the `Student` type while the only screen that would use it renders 72, so it is a record query with a list type; **`recurringHolidays`** has exactly one consumer in the product — the Expo app — and it calls the REST twin `GET /api/holidays/recurring`. `graphqlStatus` is also unselected and is meant to be: it is a wiring check. | each was built with its module's read slice, ahead of the client change that would use it | `me`: extend to what a session needs, then move `AuthProvider`. `student`: widen the type to the record the detail screen renders — the natural first half of the students Phase G slice. `recurringHolidays`: closes with debt 31, when Expo moves |
| 45 | **A school cannot rename a grade or a programme, and cannot manage mediums at all.** `updateGrade` and `updateProgramme` exist and no screen calls them — `GradesList` and `ProgrammesList` offer add and delete only, so correcting "Std 7" to "Grade 7" means deleting it, which is refused the moment a class is in it. Mediums are worse: `addMedium` has a hook nothing renders, and change/remove have no hook at all, so the languages a school teaches in can only be set by seeding. Found while giving the four structural catalogues mutations — the transport migration did not cause this, it made it legible. | the screens were built for onboarding, where a catalogue is created once and never corrected | give both lists an inline rename, and mediums a screen of its own or a section on one; the mutations are already there and guarded |
| 43 | **Two modules' HTTP surfaces are the removed setup wizard's, and no screen reaches them.** `subject_contexts` (5 writes, 3 reads) has no component consumer in any client — the chain is `subjectContextsService` → `useSubjectContexts` → nothing, and the one apparent reference is `CreateSectionModal` importing `useMediums`, which shares that hook file by accident of placement. `school_setup`'s nine write functions are the same shape: the only screen importing the service is `SetupGate`, which reads status. Both domains are **live** — `school_setup/seed_service.py` creates `SubjectContext` rows and `promote_service.py` calls `apply_for_grade` — so this is an orphaned interface, not dead logic. Found when Phase G tried to start with subject-contexts. | the admin-web setup wizard was removed when onboarding moved to the panel; its routes, client service and hooks were not | decide per module: delete the HTTP surface and client code, or give it a screen. Not a Phase G slice either way — migrating a write no user can reach is debt 18's shape |
| 42 | **The write half of the GraphQL migration is unscheduled, and the count understates it.** 40 queries against 10 mutations, and 257 REST business writes. Scoped in `phase-g-write-migration.md`: 92 of those are admin-web-only and movable without a mobile release, 82 are called by both clients and gated on the Expo release that also gates 25, 31, 2b and 4b. The count is the surface, not the work — most REST writes are CRUD while all ten existing mutations are named as acts, so each module needs modelling rather than porting. | reads were migrated first, module by module, and writes were never sequenced | answer the sequencing question in §3 of the scope, then work the order in §4 |
...
| 69 | **`teacher-leaves` has two hook APIs.** The module's screens were rebuilt onto TanStack Query, but `TeacherLeavesScreen` (the admin approval queue, which also drives balance adjustment and policy editing) and `TeacherDetailScreen` still use the original hand-rolled `useState` store in the same file, so neither invalidates after a mutation. | rebuilding three screens at once would have broken two of them in the same commit | rebuild the admin queue and teacher detail screens onto the query hooks, then delete the deprecated store and the two modals it feeds (`LeaveBalanceModal`, `LeavePolicyModal`) |
| 70 | **`tests/conftest.py`'s autouse fixtures reach `tests/` only.** The root `conftest.py` re-exports shared fixtures by name, and an autouse fixture that is not named there does not apply to `modules/*/tests/`. `_no_outbound_delivery` was added to the list; **`_no_throttling` was not**, so module test trees still run with rate limiting on and can trip the same shared counter the fixture exists to avoid. | fixing one fixture's blast radius at a time; adding `_no_throttling` changes behaviour for every module test | add `_no_throttling` to the re-export list and run the full suite, or move both fixtures to the root conftest where they belong |

---

## Closed

**`auth_events` recorded who was acted on but not who acted (debt 55, closed 2026-09-07).** Phase 1c wrote real audit rows for every credential and session operation, but the table had no actor column, so the operator went into the event's `user_agent` slot as `actor:<id>` — complete and queryable, in the wrong field, and only legible to a reader who knew the trick.

Migration 133 adds `actor_user_id` as a nullable FK with `ON DELETE SET NULL`, migrates the `user_agent LIKE 'actor:%'` rows, and `record_event` writes the column. Nullable and `SET NULL` because most auth events have no actor at all — nobody *does* a failed login — and because an operator who later leaves should not take the record of what they did with them.

The backfill migrates what it can read and **fabricates nothing**: a row written before the workaround existed keeps saying it does not know who acted, which is true. Reversible, and validated `upgrade → downgrade → upgrade` against a clone of real data.

**Examinations had no branch scoping at all (debt 56, closed 2026-08-30).** The module shipped reaching `core/branch_scope` exactly zero times while thirty-one other service files used it, so on a trust running many campuses the head of one campus could list every campus's examinations, open every campus's marking register, record marks against children they hold no authority over, and revise another campus's published result. Tenancy kept one school out of another's data; nothing kept one campus out of another's. Found by audit rather than by a ticket, which is its own lesson: the teachers/transport/hostel pass closed exactly this gap for three other domains, and the module was written afterwards without inheriting it.

The anchor is the one ADR-016 already gives: **an examination declares no campus, its papers carry the section, and the section carries the campus.** So `filter_examinations_by_branch` asks whether any sitting is in a section the user holds — which is also why one "Half Yearly" legitimately belongs to every campus of a trust at once — and papers, marks and corrections scope through `ExamPaper.class_id`, results and marksheets through the child.

Two distinctions were earned the hard way and are load-bearing:

- **A list filters, an operation on one named thing refuses.** Putting the assert inside `_paper_of` made `correction_queue` *raise* when the queue happened to hold another campus's request, instead of not showing it. Queues are now scoped at the query (`filter_by_exam_mark_ids`); single-paper and single-student operations keep the assert.
- **`papers_for` is deliberately NOT scoped, and `papers_with_labels` is.** They looked like the same read and are not: the first is the calculation's input (`applicable_papers`, `examination_cohort`, the scheduling checks), the second is the screen's. Scoping the calculation would make a student's result depend on *who pressed calculate* — a child enrolled in another campus's batch totals differently for a campus head than for the trust — and results are frozen snapshots, so that difference would be written down permanently. The docstrings on both say so.

**Still open after this**: the detail fetch (`get_examination`) is unscoped, so a campus head who knows an id can read another campus's examination header — they see no papers, sections, marks or results through it. `calculate_results` and `publish_results` act on the whole cohort by design, so a campus head holding `examination.publish` publishes trust-wide; whether that authority should be campus-bounded is a business question, not a leak, and is not answered here.

**The Examination domain had no transport (debt 47, closed 2026-08-17).** Migration 114 built the model and eight slices built the services, with nothing outside a test able to call any of it. It is reachable now, end to end: schedule an examination and fan a subject set across sections (EX-04), mark it by hand (EX-05) or from a spreadsheet (EX-06), correct a closed mark through request-and-decide (EX-07), and calculate, publish, revise and republish results (EX-08) — each on GraphQL except the two binary transfers, which are REST because that is what the conventions leave to it, and each with an admin-web screen. The one thing EX-08 added to the domain rather than exposing was `result_board`, a batched read: a results list needs student names, official-versus-current and `revision_pending`, and asking per student would have been three N+1s stacked.

**After publication there was no way to fix anything (debt 55, closed 2026-08-17).** Publication sealed the examination from every side — EX-02B refused a correction once it left `marks_entry`, EX-03A refused to recalculate a published result — which was correct and also a dead end: a school that spotted a genuine error had no path at all. EX-03C opens the one door that keeps the seal intact. A correction may now be raised against a published examination; it moves the *mark* and deliberately leaves the published figures alone, and a holder of `examination.publish` then revises the result into version N+1. Version 1 keeps its figures, its snapshot, its `published_at` and its publisher forever. The two acts stay separate — recomputing a number and telling a parent it changed are different decisions — and there is still no unpublish. Scope is one student, which is provable rather than assumed: `_compute` reads one student's marks and nothing else.

**A locked paper had no way back (debt 50, closed 2026-08-16).** Locking refused further entry, which was the point, but the correction that was supposed to be the alternative did not exist — a paper closed with a wrong mark needed the column cleared by hand. `exam_mark_corrections` (migration 118) follows `attendance_corrections` rather than inventing a second shape: state the change, require a reason, name who asked, and wait for someone holding `assessment.manage` to decide. No unlock was added, and none is needed. What the marks table forced on top of the attendance precedent is `from_marks`/`to_marks` beside the statuses, and the reason it matters is the **stale request**: two teachers can ask for the same mark to move, and approving the first turns the second's "72 → 78" into arithmetic against a number nobody is looking at any more. Recording what a request was raised *from* is what lets the approval refuse it.

**A medium could be removed out from under everything that named it (debt 44, closed 2026-08-10).** The other three structural catalogues counted their referencing classes and refused; `delete_medium` counted nothing, so a school teaching parallel English and Gujarati sections could delete "Gujarati" and leave every one of those sections pointing at a soft-deleted row. Now refused while a **class, a programme or a subject context** names it — all three, because checking only classes would let a programme keep a reference to a language the school believes it has stopped teaching in. Found by giving the four catalogues mutations, which put them side by side for the first time; the asymmetry was invisible while each lived in its own routes file.
