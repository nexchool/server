...
| 35 | **Section merge is implemented and unreachable.** `modules/classes/section_merge.py:34` implements `merge_sections` with model support already in place (`merged_into_class_id`, `merged_on`, `is_merged_away`). No route, no resolver, no script calls it. Separately, `modules/classes/services.py` never filters on those columns, so once merge is reachable, merged-away sections will keep appearing in every class picker. | the capability landed before its transport | Phase B1: expose it as a mutation with authorization, validation and audit, and exclude merged sections from active class queries in the same change |
