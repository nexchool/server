...
|---|------|---------------|------|
| 61 | **WhatsApp usage will not bill until a `whatsapp` service exists in the billing catalog.** `usage_recorder.record_provider_usage` calls `record_usage(service_key=capability)` (`modules/billing/usage.py`), which resolves the school's `TenantService` for that service key and **raises `UnknownService`** when none matches — `record_usage` never records a service key nobody has been sold. `record_provider_usage` catches exactly that exception and logs it at `ERROR`, then returns `None` rather than propagating: a failure to bill is deliberately not a failure to send, so the message that already went out does not turn into an apparent send failure. Net effect: today, before any operator has added `whatsapp` to the service catalog, every WhatsApp send — a real OTP, a test-send — succeeds, is logged once as an error nobody is paged on, and leaves no `ServiceUsageRecord`. `sms` does not have this gap only because Phase 2 already seeded an `sms` `ProviderService`/`TenantService` pair; `whatsapp` shipped in Phase 9 with no equivalent. | the billing catalog and the messaging capability list are two separate registries that happen to need the same keys, and Phase 9 added the capability without also adding the matching catalog entry and tenant subscription | a platform operator adds the service via `POST /platform/service-catalog/services` (`upsert_provider_service`, `modules/platform/routes.py`), then subscribes each WhatsApp-enabled school to it through `POST /platform/tenants/<id>/services` — until both exist for a given school, its WhatsApp usage sends but does not bill |
