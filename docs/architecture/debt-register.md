...
| 44 | **A medium can be removed out from under the classes taught in it.** The other three structural catalogues refuse deletion while a class still names them — `delete_grade` and `delete_programme` count referencing classes, `delete_school_unit` counts them and names the number. `delete_medium` counts nothing: it sets `deleted_at`, flips `is_active` and commits. A school with parallel English and Gujarati sections can delete "Gujarati" and leave every one of those sections pointing at a soft-deleted row. Medium is a dimension of a section's identity by the scale contract, so this is the one of the four where an orphan matters most. Found while giving the four catalogues mutations — the asymmetry is only visible when they sit side by side. | mediums was added later than the other three and never got the guard they share | count classes and class-subjects naming the medium and refuse with the same `referenced by existing` wording, so the transport classifies it as a CONFLICT like the others; the mutation's test lands with the fix |
