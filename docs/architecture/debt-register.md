...
| 31 | **The class, subject, student, holiday and timetable reads are on two transports at once, which the strategy forbids.** `GET /api/classes/`, `GET /api/classes/<id>`, the flat `GET /api/subjects/`, `GET /api/students/`, the three holiday reads, the two timetable reads, and the teacher list, detail, subjects, availability, workload and leave queue answer the Expo client while admin-web reads `classes` / `class` / `subjects` on GraphQL. Each REST route shares the GraphQL reader (`get_all_classes`, `class_detail`, `list_subjects_filtered`) and only reshapes it, so they cannot drift — but there are two shapes for one operation, and `legacy_detail_payload` exists solely to keep the mobile keys. | a shipped app calls them; deleting them broke it twice already | delete all of them with the Expo release that moves the mobile app to GraphQL — the same gate as 25, 2b and 4b |
...
| 33 | **Only the Teacher role holds `school_setup.read`; the School Admin holds neither setup key.** So a school administrator gets 403 on their own school's setup readiness and a teacher gets 200. admin-web works around it by gating the call on `isPlatformAdmin` and a comment calling the endpoint "super-admin-only now" — a misdiagnosis of a seeding defect. Not caused by the GraphQL move: the field carries the same authority the route did. | RBAC seeding, not the route | decide who should hold it (onboarding moved to the panel, so this may be deliberate for admins — but a Teacher holding it is not), then fix the seed and any live tenants |
