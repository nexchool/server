...
|---|------|---------------|------|
| 60 | **A migration must never interpolate a live application constant into its DDL string — replaying it later reproduces a reference, not the schema it actually produced.** Migration 129 built `ck_tenant_integrations_capability` as `f"capability IN {CAPABILITIES}"`; `CAPABILITIES` was `('sms',)` when 129 ran, so the constraint every database that applied it carries permits only `sms` forever, while `modules/integrations/models.py` builds the same constraint from the same name, now `('sms', 'whatsapp')` — model and database disagreed, and no school could be stored with a WhatsApp integration until this was found (Task 7b, Phase 9). **Closed for this instance** by migration 135, which drops and recreates the constraint with the literal `capability IN ('sms', 'whatsapp')`, and by `tests/test_integration_capability_constraint.py`, which reads the constraint's definition back off `pg_constraint` — not off the model, which would only prove itself — so a third messaging capability fails this test rather than a school's WhatsApp integration in production. The same f-string-over-a-constant mechanism, unfixed because none of it disagrees with its model yet, remains in `080_staff_relationship.py` (`employment_status`, line 54), `128_what_a_school_pays_for_and_what_it_costs_us.py` (`pricing_mode`, lines 99 and 146), 129's own sibling `ck_tenant_integrations_status` (line 93) and `130_a_code_that_lives_for_five_minutes.py` (`ck_mobile_otp_status`, line 96) — each constant there is a value frozen locally in its migration file rather than imported from the app, so none of them can drift on replay the way 129 first appeared to; each is simply one un-mirrored model change away from needing its own widening migration, the same shape 135 is. | a migration was written the way its model was, forgetting that the model is allowed to keep changing after the migration is applied and the migration file is not | when any of the four listed constraints is next touched for an unrelated reason, rewrite its f-string as a plain SQL literal for clarity (no functional bug to fix); give any status/enum column whose model builds its `CHECK` dynamically a live-constraint test shaped like `test_integration_capability_constraint.py`'s |
