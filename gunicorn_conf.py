...
# Each in-flight SSE stream (GET /api/notifications/stream) holds one thread for its whole
# lifetime, so the pool must comfortably exceed peak concurrent streams or the worker stops
# serving other requests and gunicorn's `timeout` kills it. Size GUNICORN_THREADS to expected
# concurrent SSE clients per worker + headroom for normal traffic.
